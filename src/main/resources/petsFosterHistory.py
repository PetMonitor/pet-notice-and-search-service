...
class PetTransference(Resource):

    PET_TRANSFER_SCHEMA = {
        "uuid": {"type": "string", "required": True},
        "_ref": {"type": "string", "required": True},
        "transferToUser": {"type": "string", "required": True},
    }

    def __init__(self):
        # Argument parser for PetTransference creation's JSON body
        self.arg_validator = Validator()
        self.arg_validator.allow_unknown = False
        super(PetTransference, self).__init__()
    ...
    def post(self, petId):
        ...
        try:
            ...
            if not self.arg_validator.validate(petTransferData, PetTransference.PET_TRANSFER_SCHEMA):
                print("ERROR {}".format(self.arg_validator.errors))
